...
> The server binds `127.0.0.1:7989` (localhost only) by default and has no auth.
Set `host` to `0.0.0.0` in `cleanup_config.json` to expose it on the LAN — trusted networks only.
"""
